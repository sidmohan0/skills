...
description: Read terminal and shell project context, including current directory, git branch and status, recent shell commands, likely running developer processes, active virtualenv or conda environment, and tmux sessions. Use when the user asks what they are coding, what terminal project is active, what commands were run, what jobs are running, or when another skill needs coding context before acting.
---
...
Use shell, git, process, history, and tmux state as the source of truth. This skill is read-only and should not start, stop, or attach to jobs.
...
Default behavior reports context for the process current directory.
...
- A non-interactive script cannot read live shell builtins like `jobs` from another terminal; report process candidates and tmux state instead.
